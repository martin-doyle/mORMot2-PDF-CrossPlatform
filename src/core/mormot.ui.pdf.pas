...
uses
  ...
  mormot.core.buffers,
  mormot.core.data,
  Contnrs;  // TObjectList (for fStructElems)
...
{ Windows charset/font constants - defined here for cross-platform compilation }
const
  ANSI_CHARSET         = 0;
  DEFAULT_CHARSET      = 1;
  SYMBOL_CHARSET       = 2;
  ...
  FW_BOLD              = 700;
  TMPF_FIXED_PITCH     = 1;
  LOGPIXELSY           = 90;  // device capability index - unused on POSIX
...
{$ifdef USE_PDFSECURITY}
  ...
  // - RC4 40-bit and RC4 128-bit correspond to PDF 1.3 and PDF 1.4 formats
  // - elAES_128 uses AES-128-CBC (PDF 1.6, Standard Security Handler R=4)
  // - elAES_256 uses AES-256-CBC (PDF 1.7, Standard Security Handler R=6)
  // - for RC4 and AES, passwords are restricted to 32 Latin-1 characters
  TPdfEncryptionLevel = (
    elNone,
    elRC4_40,
    elRC4_128,
...
{$ifdef USE_PDFSECURITY}
  ...
  public
    ...
    /// returns the byte size of the encrypted output for a given input size
    // - default: returns InSize (RC4 is a stream cipher, same-length output)
    // - AES subclasses override to return 16 + ceil(InSize/16)*16 (IV + padded)
    function GetEncryptedSize(InSize: cardinal): cardinal; virtual;
    ...
    // - elRC4_40, elRC4_128, and elAES_128 levels are implemented
    ...
    // ! Doc := TPdfDocument.Create(false,0,false,
    // !   TPdfEncryption.New(elAES_128,'','toto',PDF_PERMISSION_NOCOPYNORPRINT));
  ...
  protected
    ...
    procedure EncodeBuffer(const BufIn; var BufOut; Count: cardinal); override;
  public
    function GetEncryptedSize(InSize: cardinal): cardinal; override;
    procedure AttachDocument(aDoc: TPdfDocument); override;
  end;
{$endif USE_PDFSECURITY}
...
{************ TPdfDocument TPdfPage main rendering classes }
  ...
  /// one embedded font file, shared by every font with byte-identical data
  // - several TPdfFontTrueType instances may resolve to the very same physical
  // font file - e.g. the Regular and Bold styles of a .ttc collection face on
  // POSIX, where only face index 0 is reachable - and would otherwise embed
  // the same megabytes once per instance
  TPdfFontFile2 = record
    /// crc32c of Data, used as a fast pre-filter before the full comparison
    Hash: cardinal;
  ...
  public
    ...
    /// the native resolution of the PDF in DPI (default: 96)
    // - used for coordinate conversion between screen/report units (1/100 mm) and PDF points
    // - typically set to GetDeviceCaps(hdc, LOGPIXELSY) on Windows
    // - PDF always uses 72 points per inch internally, but this value determines the scaling factor
    // - CRITICAL: RenderPageToCanvas uses this value to ensure PDF export matches screen preview
    ...
    /// select the PDF version written to the file header (%PDF-1.x)
    // - default is pdf13; pdf17 = ISO 32000-1 (2008)
    ...
    property Tagged: boolean
      read fTagged write SetTagged;
    /// BCP-47 language tag written to the /Lang catalog entry when Tagged=true
    // - default 'en'; set before the first AddPage call
    property DefaultLanguage: RawUtf8
  ...
  protected
    ...
    fCurrentMCID: integer;    // per-page MCID counter, reset to 0 in AddPage
    fStructParents: integer;  // StructParents key for ParentTree (-1 = not tagged)
  ...
  public
    ...
    /// retrieve or set the paper orientation (landscape = width > height)
    // - setting this property swaps the current page width and height
    // - default (false) = portrait orientation (height > width)
    // - affects the page's coordinate system and rendering layout
  ...
  public
    ...
    /// Set the fill (non-stroking) opacity via an ExtGState graphics state
    // - Value is clamped to [0..1]: 0=fully transparent, 1=fully opaque
    // - emits a named ExtGState entry (/ca operator) into page resources
    procedure SetFillAlpha(Value: single);
    /// Set the stroke opacity via an ExtGState graphics state
    // - Value is clamped to [0..1]: 0=fully transparent, 1=fully opaque
    // - emits a named ExtGState entry (/CA operator) into page resources
    ...
    // - useful for creating multi-layer PDFs where content can be toggled on/off
    procedure BeginMarkedContent(Group: TPdfOptionalContentGroup);
    /// ends optional content (layer)
    // - must be called after BeginMarkedContent to properly close the marked content section
    // - mismatch of Begin/End pairs will result in invalid PDF
    procedure EndMarkedContent;
    /// open a Tagged PDF marked content section (BDC operator)
    // - only writes when TPdfDocument.Tagged=true
    // - auto-increments the per-page MCID counter and registers the struct element
...
begin
  ...
  {$else}
  result := 1; // on POSIX, MBCS via Windows API is not available
  {$endif OSWINDOWS}
end;
...
begin
  ...
  if (H < 24) and
    ...
    // the time of day is a fraction of a day: divide, don't multiply
...
begin
  ...
  try
    ...
    {$ifdef USE_PDFSECURITY}
    if (buflen > 0) and (W.fDoc.fEncryption <> nil) and not fDoNotEncrypt then
      encLen := W.fDoc.fEncryption.GetEncryptedSize(buflen)
    else
      encLen := cardinal(buflen);
    len.Value := integer(encLen);
    {$else}
    len.Value := buflen;
    {$endif USE_PDFSECURITY}
    if fFilter <> '' then
    ...
    if (buflen > 0) and (W.fDoc.fEncryption <> nil) and not fDoNotEncrypt then
    begin
      if encLen = cardinal(buflen) then
        // RC4: in-place encryption, same output size
...
begin
  if Text <> '' then
    ...
    begin
      tmp.Init(integer(fDoc.fEncryption.GetEncryptedSize(length(Text))));
      fDoc.fEncryption.EncodeBuffer(pointer(Text)^, tmp.buf^, length(Text));
...
begin
  ...
  begin
    ...
    if fDoc.fEncryption <> nil then
    begin
      sectmp.Init(integer(fDoc.fEncryption.GetEncryptedSize(WideCharCount * 2)));
      fDoc.fEncryption.EncodeBuffer(PW^, sectmp.buf^, WideCharCount * 2);
      PW := sectmp.buf;
      WideCharCount := sectmp.len div 2; // adjust for AES-padded output
    end;
    {$endif USE_PDFSECURITY}
    repeat
      L := WideCharCount;
...
begin
  ...
  idx := Ttf.FindOrAddUsedWideChar(Char); // grow fUsedWide before reading it
...
begin
  ...
  for i := 0 to Header^.numberSubtables - 1 do
    with SubTable^[i] do
      ...
      else if platformID = TTFCFP_UNICODE_PLATFORMID then
        // Apple fonts may expose no Microsoft subtable at all - e.g. the first
        // faces of macOS GeezaPro.ttc only have (0,3) - so keep the Unicode
        // platform map as a fallback, its format 4 layout being identical
        if platformSpecificID = TTFCFP_UNICODE_BMP then
          uni := offset // preferred: the BMP map is the format 4 one
        else if (platformSpecificID = TTFCFP_UNICODE_FULL) and
                (uni = 0) then
          uni := offset;
  if off = 0 then
  begin
    off := uni; // no Microsoft subtable: fall back to the Unicode platform
    aUnicodeTtf.fIsSymbolFont := false;
  end;
  if off = 0 then
    exit; // no subtable we could use
  ...
  if off and 1 <> 0 then
  begin
    // odd offset: GetTtfData() applied SwapBuffer() on even-aligned 16-bit
    // words, so every read from here would straddle two swapped words and
    // return garbage (e.g. macOS 'Hiragino Sans GB' has its (3,1) subtable at
    // offset $A5B).  Undo that swap, then redo it from the odd boundary - same
  ...
  // UnitsPerEm is typically a power of 2 but may be any value 16..16384
  // (e.g. Noto Naskh Arabic uses UPM=1000); use exact division to avoid
  // widths being 1.953x too large for non-power-of-2 fonts
  if head^.UnitsPerEm > 0 then
  begin
    W := (int64(fhmtx[0]) * 1000) div head^.UnitsPerEm;
        ...
        with aUnicodeTtf.fUsedWide[i] do
          if Glyph <> 0 then
            if Glyph <= numOfLongHorMetrics then
              Width := (int64(fhmtx[Glyph * 2]) * 1000) div head^.UnitsPerEm
            else
              Width := W;
    end;
  end;
...
begin
  ...
  if (i < 0) or (i >= length(UnicodeFont.fUsedWide)) then
    i := 0 // glyph absent from CMAP or invariant broken -> use .notdef
  else
...
begin
  ...
  with UnicodeFont do // UnicodeFont.fUsedWide[] = available glyphs from TPdfTtf
    for i := 0 to fUsedWideChar.Count - 1 do
      if fUsedWide[i].Glyph = aGlyph then
      begin
        // split to avoid evaluation-order AV: FindOrAddUsedWideChar may call
        // SetLength(fUsedWide,...) which moves the array; pre-loading the base
        // pointer before the call would leave a stale reference (see Fix C).
...
begin
  ...
  // common Linux/macOS fonts without 'Unicode' in the name
  if fFontFallBackIndex < 0 then
    fFontFallBackIndex := GetTrueTypeFontIndex('DejaVu Sans');
  if fFontFallBackIndex < 0 then
    fFontFallBackIndex := GetTrueTypeFontIndex('Liberation Sans');
  if fFontFallBackIndex < 0 then
    fFontFallBackIndex := GetTrueTypeFontIndex('FreeSans');
  if (fFontFallBackIndex < 0) and (fTrueTypeFonts <> nil) then
    fFontFallBackIndex := 0; // last resort: first available font
  NewDoc;
end;
...
begin
  ...
  // Tagged PDF: set up struct tree on first page and StructParents per page
  if fTagged then
  begin
    result.fStructParents := fRawPages.Count - 1;
    result.fCurrentMCID := 0;
    result.AddItem('StructParents', fRawPages.Count - 1);
    if fStructTree = nil then
    begin
      // lazy catalog setup: add MarkInfo, Lang, StructTreeRoot on first AddPage
      ...
      begin
        ...
        begin
          ...
          mormot.core.text.BinToHex(PAnsiChar(@fFileID), P + 1, 16);
          P[SizeOf(fFileID) * 2 + 1] := '>';
          xrefID := TPdfArray.Create(fXRef);
          xrefID.AddItem(TPdfRawText.Create(hexFileID));
          xrefID.AddItem(TPdfRawText.Create(hexFileID));
          fTrailer.Attributes.AddItem('ID', xrefID);
        end;
      end;
    end;
  end;
...
type
  ...
  public
    Role: TPdfStructRole;
    /// page index the element starts on, written as /Pg
    PageIndex: integer;
    /// marked-content identifiers owned by this leaf, none for a container
    // - a logical block interrupted by a page break owns one MCID per page,
    // which ISO 32000-1 14.7.4.3 allows as long as each MCR names its /Pg
    MCIDs: TIntegerDynArray;
    /// page index of each MCIDs[] entry
...
var
  ...
  begin
    result := TPdfDictionary.Create(fXRef);
    result.AddItem('Type', 'MCR');
    pg := aElem.MCIDPages[aIndex];
    if (pg <> aElem.PageIndex) and
...
begin
  ...
  begin
    ...
    if (not PDF_STRUCT_CONTAINER[elem.Role]) and
      ...
      else if elem.MCIDCount > 1 then
      begin
        // one logical block, several regions (page break): /K is an MCR array
        mcrArr := TPdfArray.Create(fXRef);
        for j := 0 to elem.MCIDCount - 1 do
          mcrArr.AddItem(NewMCR(elem, j));
        elem.Dic.AddItem('K', mcrArr);
      end;
  end;
  ...
  // - only leaves own MCIDs, so the array index equals the MCID
  // - an element continued after a page break appears in both pages' arrays
  maxPage := 0;
  for i := 0 to fStructElems.Count - 1 do
  begin
    elem := TPdfStructElement(fStructElems[i]);
    if elem.PageIndex > maxPage then
      maxPage := elem.PageIndex;
    for j := 0 to elem.MCIDCount - 1 do
      if elem.MCIDPages[j] > maxPage then
        maxPage := elem.MCIDPages[j];
  end;
  numsArr := TPdfArray.Create(fXRef);
  for pageIdx := 0 to maxPage do
  begin
    maxMCID := 0;
    for i := 0 to fStructElems.Count - 1 do
    begin
      elem := TPdfStructElement(fStructElems[i]);
      for j := 0 to elem.MCIDCount - 1 do
        if (elem.MCIDPages[j] = pageIdx) and
           (elem.MCIDs[j] >= maxMCID) then
          maxMCID := elem.MCIDs[j] + 1;
    end;
    if maxMCID = 0 then
      continue;
    ...
    begin
      ...
      for i := 0 to fStructElems.Count - 1 do
      begin
        for j := 0 to TPdfStructElement(fStructElems[i]).MCIDCount - 1 do
          if (TPdfStructElement(fStructElems[i]).MCIDPages[j] = pageIdx) and
             (TPdfStructElement(fStructElems[i]).MCIDs[j] = mcid) then
          begin
            elem := TPdfStructElement(fStructElems[i]);
            break;
          end;
...
var
  ...
  h: cardinal;
begin
  h := crc32c(0, pointer(aTtf), length(aTtf));
...
begin
  ...
  if not PdfCanSubsetRetainingGids or
...
begin
  // Standard PDF fonts (14 base fonts) are charset-independent: the PDF spec
  // defines them regardless of locale/codepage. Don't block them for non-ANSI charsets.
...
const
  ...
  STAND_FONTS_PDF: array[TPdfFontStandard] of RawUtf8 = (
    PDF_FONT_STD_SERIF,   // pfsTimes
    PDF_FONT_STD_SANS,    // pfsHelvetica
    PDF_FONT_STD_MONO);   // pfsCourier
...
begin
  ...
  begin
    ...
    // Standard PDF fonts (the 14 base fonts) work in any charset context,
    // so don't require ANSI_CHARSET check. They are defined by the PDF spec itself.
    if fDoc.StandardFontsReplace then
    begin
      for f := low(f) to high(f) do
        if SameTextU(AName, STAND_FONTS_PDF[f]) or
           SameTextU(AName, STAND_FONTS_WIN[f]) then
    ...
    begin
      ...
      begin // unknown, device or raster font
        ...
        begin // use variable width default font
          ...
          if ndx < 0 then
            ndx := fDoc.GetTrueTypeFontIndex('DejaVu Sans');
          if ndx < 0 then
            ndx := fDoc.GetTrueTypeFontIndex('Liberation Sans');
          if (ndx < 0) and (fDoc.fTrueTypeFonts <> nil) then
            ndx := 0; // last resort: first available font
          if ndx < 0 then
            exit;
        end;
...
procedure TPdfCanvas.SetFillAlpha(Value: single);
var
  gsName: PdfString;
  extGs: TPdfDictionary;
...
begin
  ...
  gsName := 'Fca' + PdfString(IntToStr(intVal));
  extGs := fPage.GetResources('ExtGState');
  if extGs = nil then
  begin
    extGs := TPdfDictionary.Create(fDoc.fXRef);
    fPage.PdfDictionaryByName('Resources').AddItem('ExtGState', extGs);
  end;
  if extGs.ValueByName(gsName) = nil then
  begin
    gsDict := TPdfDictionary.Create(fDoc.fXRef);
    gsDict.AddItem('Type', 'ExtGState');
    gsDict.AddItem('ca', TPdfReal.Create(Value));
    extGs.AddItem(gsName, gsDict);
  end;
  fContents.Writer.Add('/').Add(gsName).Add(' gs'#10);
end;

procedure TPdfCanvas.SetStrokeAlpha(Value: single);
var
  gsName: PdfString;
  extGs: TPdfDictionary;
  gsDict: TPdfDictionary;
...
begin
  ...
  if Value < 0 then
    ...
    Value := 1;
  intVal := Round(Value * 100);
  gsName := 'SCA' + PdfString(IntToStr(intVal));
  extGs := fPage.GetResources('ExtGState');
  if extGs = nil then
  begin
    extGs := TPdfDictionary.Create(fDoc.fXRef);
...
procedure TPdfCanvas.BeginStructContent(ARole: TPdfStructRole;
  const AAltText: RawUtf8);
var
  mcid: integer;
  elem: TPdfStructElement;
...
begin
  if (fContents = nil) or
     not fDoc.fTagged or
     (fDoc.fStructElems.Count = 0) then
    result := -1
  else
...
begin
  if (fContents = nil) or not fDoc.fTagged then
    exit;
  if cardinal(AStructIndex) >= cardinal(fDoc.fStructElems.Count) then
    raise EPdfInvalidOperation.CreateUtf8(
      'ResumeStructContent(%): no such struct element', [AStructIndex]);
  elem := TPdfStructElement(fDoc.fStructElems[AStructIndex]);
  if PDF_STRUCT_CONTAINER[elem.Role] then
    raise EPdfInvalidOperation.CreateUtf8(
  ...
  case aLevel of
    elRC4_40,
      ...
      result := TPdfEncryptionRC4MD5.Create(aLevel, aPermissions, aUserPassword,
        aOwnerPassword);
    elAES_128:
      result := TPdfEncryptionAES128.Create(aLevel, aPermissions, aUserPassword,
        aOwnerPassword);
  else
    result := nil;
  end;
...
function TPdfEncryption.GetEncryptedSize(InSize: cardinal): cardinal;
begin
  result := InSize; // stream cipher: output = input size
end;
...
{ TPdfEncryptionAES128 }

function TPdfEncryptionAES128.GetEncryptedSize(InSize: cardinal): cardinal;
begin
  // IV (16 bytes) + PKCS7-padded ciphertext (always rounds up to next 16-block)
  result := 16 + ((InSize div 16) + 1) * 16;
end;

procedure TPdfEncryptionAES128.AttachDocument(aDoc: TPdfDocument);
...
const
  AES128_FLAGS  = cardinal($FFFFF0C0); // AES-128 uses same flag pattern as RC4-128
  FLAGBIT: array[TPdfEncryptionPermission] of byte = (
    2, 3, 4, 5, 8, 9, 10, 11);
var
  rc4: TRC4;
  ...
  i, j: integer;
  own, usr: TPdfBuffer32;
  cfDict, stdCf: TPdfDictionary;
begin
  inherited;
  // compute permission flags (same bit layout as RC4)
  fFlags := integer(AES128_FLAGS);
  for f := low(f) to high(f) do
    if f in fPermissions then
      fFlags := fFlags or (1 shl FLAGBIT[f]);
  if aDoc.fFileFormat < pdf16 then
    aDoc.fFileFormat := pdf16; // AES-128 requires PDF 1.6+
  // derive owner key (Algorithm 3.3, step 1-3 identical for all R>=2)
  Pad(fUserPassword, usr);
  Pad(fOwnerPassword, own);
  md5.Full(@own, SizeOf(own), dig);
  for i := 1 to 50 do
    md5.Full(@dig, SizeOf(dig), dig);
  rc4.Init(dig, 16); // 128-bit key
  rc4.Encrypt(usr, fOwnerPass, SizeOf(fOwnerPass));
  for i := 1 to 19 do
  begin
    for j := 0 to high(dig2) do
  ...
  // compute 128-bit file encryption key (Algorithm 3.2 with 16-byte key)
  md5.Init;
  md5.Update(usr, SizeOf(usr));
  md5.Update(fOwnerPass, SizeOf(fOwnerPass));
  md5.Update(fFlags, SizeOf(fFlags));
  ...
  for i := 1 to 50 do
    md5.Full(@dig, SizeOf(dig), dig);
  SetLength(fInternalKey, 16);
  MoveFast(dig, fInternalKey[0], 16);
  // compute /U value (Algorithm 3.5 for R=4, RC4-encrypted padding)
  md5.Init;
  md5.Update(PDF_PADDING, SizeOf(PDF_PADDING));
  md5.Update(aDoc.fFileID, SizeOf(aDoc.fFileID));
  md5.final(dig);
  ...
  MoveFast(dig, fUserPass, SizeOf(dig));
  MoveFast(dig, fUserPass[SizeOf(dig)], SizeOf(dig));
  // build encryption dictionary (V=4, R=4, AES-128 with CF/StmF/StrF)
  if aDoc.fEncryptionObject = nil then
    aDoc.fEncryptionObject := TPdfDictionary.Create(aDoc.fXRef);
  cfDict := TPdfDictionary.Create(aDoc.fXRef);
  stdCf := TPdfDictionary.Create(aDoc.fXRef);
  stdCf.AddItem('AuthEvent', 'DocOpen');
  stdCf.AddItem('CFM', 'AESV2');
  stdCf.AddItem('Length', 16);
  cfDict.AddItem('StdCF', stdCf);
  with aDoc.fEncryptionObject do
  begin
    AddItem('Filter', 'Standard');
    AddItem('V', 4);
    AddItem('R', 4);
...
var
  ...
  aes: TAesCbc;
  iv: TAesBlock;
  padLen, padByte: cardinal;
  padBuf: PByteArray;
  outP: PByte;
begin
  // derive per-object 128-bit key (Algorithm 3.1a)
  md5.Init;
  md5.Update(fInternalKey[0], 16);
  md5.Update(fDoc.fCurrentObjectNumber, 3);
  md5.Update(fDoc.fCurrentGenerationNumber, 2);
  md5.Update(AES_SALT, SizeOf(AES_SALT));
  md5.final(dig);
  // generate random IV
  RandomBytes(@iv, SizeOf(iv));
  outP := @BufOut;
  MoveFast(iv, outP^, SizeOf(iv));
  inc(outP, SizeOf(iv));
  // PKCS7-pad the plaintext into a scratch buffer
  padByte := 16 - (Count and 15); // 1..16 padding bytes
  padLen := Count + padByte;
  GetMem(padBuf, padLen);
  try
    MoveFast(BufIn, padBuf^, Count);
    FillCharFast(padBuf^[Count], padByte, padByte);
    // AES-128-CBC encrypt with derived key and random IV
    aes := TAesCbc.Create(dig, 128);
    try
      aes.IV := iv;
      aes.Encrypt(padBuf, outP, padLen);
    finally
      aes.Free;
    end;
  finally
